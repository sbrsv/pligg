...
if (!$errors) {
	...
	$sql = "INSERT INTO `" . table_users . "` (`user_id`, `user_login`, `user_level`, `user_modification`, `user_date`, `user_pass`, `user_email`, `user_names`, `user_karma`, `user_url`, `user_lastlogin`, `user_ip`, `user_lastip`, `last_reset_request`, `last_email_friend`, `user_enabled`) VALUES (1, '".$db->escape($_POST['godlogin'])."', 'god', now(), now(), '$saltedpass', '".$db->escape($_POST['godemail'])."', '', '10.00', 'http://pligg.com', now(), '0', '0', now(), now(), '1');";
	$db->query( $sql );
